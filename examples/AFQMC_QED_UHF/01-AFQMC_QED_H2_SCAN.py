...
if __name__ == "__main__":
    ...
    np.savetxt("%s/01-AFQMC_H2_SCAN_FCI_HF_basis_%s.dat" % (DATA_DIR,basis), np.c_[bond_list_fine, E_rHF, E_uHF, E_FCI], fmt="%1.6f", header="R_HH  E_rHF  E_uHF  E_FCI")
    ...
    plt.plot(bond_list_fine,   E_rHF , '-', c='red', lw=2, label="HF")
    ...
    plt.errorbar(bond_list_coarse, E_uAFQMC_AVE, yerr=E_uAFQMC_STD, fmt='s', linestyle="solid", elinewidth=1, ecolor='black', capsize=4, c='black', mfc='none', label="AFQMC@UHF")
    ...
    plt.clf()
    np.savetxt("%s/01-AFQMC_H2_SCAN_T_%1.0f_dt_%1.4f_Nw_%1.0f_basis_%s_LAM_%1.3f.dat" % (DATA_DIR,total_time,dt,num_walkers,basis,cavity_coupling[0]), np.c_[bond_list_coarse, E_rAFQMC_AVE, E_uAFQMC_AVE, E_rAFQMC_AVE_QED, E_uAFQMC_AVE_QED, E_rHF_QED], fmt="%1.6f", header="R_HH  AFQMC@RHF AFQMC@UHR  QED-AFQMC@RHF  QED-AFQMC@UHF  QED-RHF")
